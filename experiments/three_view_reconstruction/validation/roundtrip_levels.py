...
from parser.projection_mapping import map_lines_to_frame
...
def split(level_2: dict, semantic_graph: dict | None = None, expected_graph=None) -> dict:
    ...
    geometry_complete = bool(views) and all(
        ...
        and v.get("arcs", {"status": "PASS"}).get("status") == "PASS"
    ...
    hidden_matches = {}
    expected_views = (expected_graph.front, expected_graph.top, expected_graph.left) if expected_graph else ()
    need_hidden_match = bool(expected_graph and hasattr(expected_graph, "projection") and all(hasattr(view, "hidden_segments") for view in expected_views))
    if need_hidden_match:
        contracts = (("front", expected_graph.front, CanonicalViewOrientation.FRONT),
                     ("top", expected_graph.top, CanonicalViewOrientation.TOP),
                     ("left", expected_graph.left, CanonicalViewOrientation.RIGHT))
        semantic_by_role = {str(v.get("orientation", {}).get("canonical_role")): v for v in semantic_graph.get("views", [])}
        for role, expected_view, target_role in contracts:
            if not expected_view.hidden_segments:
                continue
            actual_view = semantic_by_role.get(target_role.value, {})
            actual_hidden = [item.get("geometry", {}) for item in actual_view.get("projected_geometry", {}).get("hidden", []) if item.get("geometry_type") == "LINE"]
            frame = canonicalize(target_role.value, expected_graph.projection.upper())
            expected_hidden = map_lines_to_frame(role, [segment.__dict__ for segment in expected_view.hidden_segments], expected_view.horizontal_extent, expected_view.vertical_extent, frame)
            hidden_matches[role] = match_line_supports(expected_hidden, actual_hidden)
    hidden_complete = (not need_hidden_match) or all(item["status"] == "PASS" for item in hidden_matches.values())
    provenance_ok = semantic_graph.get("status") == "PASS" and semantic_graph.get("semantic_provenance") == "HLV_MINUS_HLR" and unknown_count == 0 and hidden_complete
    ...
    reasons = []
    if not provenance_ok: reasons.append("SEMANTIC_PROVENANCE_OR_HIDDEN_GEOMETRY_INCOMPLETE")
    ...
    level_2b = {"status": level_2b_status, "scope": "visible/hidden/centerline/centermark semantics",
                ...
                "hidden_geometry_matches": hidden_matches,
