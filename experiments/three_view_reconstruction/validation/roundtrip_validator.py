...
def validate(input_graph: ProjectionGraph, features: FeatureGraph, backend: dict, tolerance_mm: float = 0.01) -> dict:
    ...
    return {"status": "PASS" if all(c["passed"] for c in checks+hole_checks+[drawing_check]) else "FAIL", "mode": "structured_projection_invariants_plus_generated_drawing_structure", "limitations": ["v0.1 does not yet extract linework/hidden lines from regenerated SLDDRW", "therefore this is not pixel-level or drawing-entity-level equality"], "view_checks": checks, "hole_checks": hole_checks, "drawing_check": drawing_check}
